...
- **A round ends when nothing can still act, not when the board is empty.** A Knight on the back rank has no legal move ever again — every hop needs `rank − 2` or `rank − 1`, so from rank 0 all four candidates land off the board, and a Knight has no lateral fallback to catch it. Waiting for an empty board hangs the round forever.
...
- **Pieces are forward-biased and deterministic.** Direction is a pure function of Piece type, `moveCount`, and `handedness`. Never choose a line because the Core is on it — that is goal-seeking, and it makes Tower placement steer Pieces.
