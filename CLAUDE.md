...
Packs have landed, closing the gap between design and code on the economy — pricing and King/Ace caps are still open, so read `docs/design/game-design.md` for the intended design rather than inferring it from what is built. The largest unbuilt pieces now are the pack-opening animation and an enterable seed.
...
- **Hands are gap-only, and two-step.** A hand purchase (`playHand`) is refused mid-round and even while another hand is pending; the Tower appears awaiting placement (`pendingTower`, which carries the committed Cards), and `placeTower` puts it down on a legal square or `cancelPlacement` drops it and **refunds the Cards to the Deck** — cancelling is a full undo, and only an explicit cancel refunds: an illegal placement keeps the pending Tower and the Cards stay committed. Face-card actions and the Joker's Clear are the deliberate exceptions: they are playable any time, exactly as before.
