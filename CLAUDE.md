...
- **Across the ranks that actually fire, single-target DPS never rises and `targetsPerShot` never falls — but coverage is not monotonic with rank at all, and neither claim may reach past the Wall.** Rank 7 fires nothing, so its `damage` and `targetsPerShot` are both 0 — DPS visibly *rises* from rank 7 to rank 8 (0 → 1.43), and a "never rises with rank" claim that includes the Wall is already false. `src/data/towerRanks.test.ts` scopes both invariants to `FIRING_RANKS` (every rank but the Wall) for exactly this reason. Coverage is looser still: measured peak coverage on an 8x8 board is 8, 7, 14, 13, 24, 0, 39, 24, 23 for ranks 2 through 10, falling at 2→3, 4→5, 6→7, 8→9 and 9→10 — rank 10 covers fewer squares than rank 6. What actually holds, and what `towerRanks.test.ts` pins, is a **per-height ceiling**: no rank may cover more than 39 of an 8x8's other 63 squares, or 47 once board growth removes the rank-clipping that `vertical`, `cross`, `diagonal` and `ring` start with. Raising damage or shortening a fire interval at the top without cutting coverage to match rebuilds issue #19 exactly — a single rank-6 Tower once carried auto-rounds for 45+ rounds unattended. These are the design, not tuning.
