...
- 591 tests across 35 files, all passing, none of which need a browser. Run `pnpm test:run` for the live count — this figure is indicative of scale, and a stale one here has already leaked into a plan document once.
...
- **Pieces spawn onto the Staging rank, and it must stay out of bounds.** `stagingRank` in `src/game/board.ts` returns `board.ranks`, and the entire fix for issue #22 rests on `isInBounds` being false there and on `board.ranks` never decreasing — those, and nothing else, are what stop a Tower being built where a Piece appears. Widening `isInBounds` to include it, or shrinking `board.ranks`, would silently re-open the collision; nothing shrinks it today — `expandBoard` in `src/game/cardPlays.ts` is the only writer of `board.ranks`, and it only adds. A Piece standing there is otherwise ordinary: Towers may fire at it, a Joker's Clear destroys it, auras reach it. `src/game/staging.test.ts` pins the out-of-bounds property and those ordinary-Piece behaviours — not the monotonic-growth clause above, which is structural, guaranteed by there being no shrink writer, rather than tested.
