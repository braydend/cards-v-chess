 ...
 * through would build a Card whose rank is a string, which `commandFor` can
 * neither build nor act on: it is not a number (so not buildable) and not a
 * face rank (so no switch case matches). Face ranks survive as themselves.
 *
