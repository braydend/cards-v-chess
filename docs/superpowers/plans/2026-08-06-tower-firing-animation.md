...
**Why a cooldown decrease is an exact shot signal — with one guarded exception.** On exit from `fireTowers` in `src/game/tick.ts`, a Tower's stored `fireCooldownMs` is either below `fireIntervalMs` (it fired, subtracting one whole interval) or exactly `fireIntervalMs` (nothing in range, clamped to "ready" rather than banking shots). Ordinarily that clamp can only hold or raise the stored value, so a decrease means a shot.

**♦ Speed defeats that on its own.** `applySupport` in `src/game/support.ts` lowers `fireIntervalMs` directly and never touches `fireCooldownMs`. A Tower idling at the OLD clamp is left holding a cooldown *above* its NEW interval, and the very next tick — even with nothing in range — clamps that value DOWN to the new interval: a decrease with no shot behind it. Reproduced through the real engine: a rank 2 Tower, a Pawn out of range, a 10♦ played on the Tower — stored cooldown/interval went 600/600 → (♦) → 600/500 → next tick 500/500, a phantom decrease. `detectShots` guards this explicitly (see Step 3's code block below): it skips a decrease only when the *previous* reading already exceeded the *current* interval and the *new* reading has merely caught back up to it — something a tick that actually fired cannot produce, because firing always drains the cooldown strictly below the interval.
