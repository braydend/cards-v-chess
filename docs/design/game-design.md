...
**A Piece grinding a Tower from the Staging rank is never the exception to that asymmetry — it is the general case.** Damage cannot reach the Staging rank at all, regardless of a Tower's geometry, so a Tower whose coverage would otherwise reach back into the Staging rank still cannot kill the Piece grinding it from there. That standoff ends only when the Tower falls — the finite in-round ♥ supply is the sole bound on it, exactly as it is for an on-board grind, just without a geometry-driven shortcut ever cutting it short.
